...
Code for reproducing the MNIST experiment with DVAE
"""
import numpy as np
import torchvision
...
def create_doubles(frames_list):
    ...
    for frms in frames_list:
        ...
        dframes_list.append(np.tile(frms, (1, 1, 2, 1)))
    dframes = np.concatenate(
            dframes_list, axis=1).reshape(-1, 1, frms.shape[2], frms.shape[3])
    return dframes[1:-1].reshape(-1, 2, frms.shape[2], frms.shape[3])
